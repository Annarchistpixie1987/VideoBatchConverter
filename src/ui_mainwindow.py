...
from PyQt6.QtCore import QUrl, QTimer, QThreadPool, QMutex
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.input_browse_btn = QPushButton("찾아보기...")
        ...
        self.resolution_combo = QComboBox(); self.resolution_combo.addItems(['원본 유지', '1080p', '720p'])
        preset_items = ['p1 (빠름, 낮은 품질)', 'p2', 'p3', 'p4 (중간)', 'p5', 'p6', 'p7 (느림, 고품질)']; self.preset_option_combo = QComboBox(); self.preset_option_combo.addItems(preset_items)
        ...
        self.parallel_jobs_spinbox = QSpinBox(); self.parallel_jobs_spinbox.setMinimum(1); self.parallel_jobs_spinbox.setMaximum(self.thread_pool.maxThreadCount())
        self.overall_progress_bar = QProgressBar(); self.overall_progress_bar.setTextVisible(True); self.overall_progress_bar.setFormat("%v / %m")
        ...
        # --- 컨트롤 목록 정의 ---
        self.detail_controls = [self.codec_combo, self.resolution_combo, self.preset_option_combo, self.rate_control_combo, self.quality_edit, self.suffix_edit, self.audio_option_combo, self.output_format_combo, self.parallel_jobs_spinbox]
        self.controls = [self.input_path_edit, self.input_browse_btn, self.preset_combo, self.start_button] + self.detail_controls
        
        # --- 시그널 연결 ---
        self.preset_combo.addItems(list(PRESETS.keys())); self.preset_combo.currentTextChanged.connect(self.apply_preset)
        self.input_browse_btn.clicked.connect(self.browse_input_folder); self.start_button.clicked.connect(self.start_conversion); self.stop_button.clicked.connect(self.stop_conversion)
        ...
        self.codec_combo.currentTextChanged.connect(self.update_suffix); self.codec_combo.currentTextChanged.connect(self.update_encoder_options)
        self.rate_control_combo.currentTextChanged.connect(self.update_quality_label)

        # --- 레이아웃 구성 ---
        preset_group = QGroupBox("프리셋 선택"); preset_layout = QHBoxLayout(preset_group); preset_layout.addWidget(QLabel("프리셋:")); preset_layout.addWidget(self.preset_combo); main_layout.addWidget(preset_group)
        path_group = QGroupBox("변환 대상 폴더"); path_layout = QVBoxLayout(path_group)
        input_layout = QHBoxLayout(); input_layout.addWidget(QLabel("대상 폴더:")); input_layout.addWidget(self.input_path_edit); input_layout.addWidget(self.input_browse_btn); path_layout.addLayout(input_layout); main_layout.addWidget(path_group)
        settings_group = QGroupBox("세부 설정"); main_settings_layout = QHBoxLayout(settings_group)
        self.left_form_layout = QFormLayout(); self.left_form_layout.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapAllRows)
        self.left_form_layout.addRow(self.create_help_label("비디오 코덱", HELP_TEXTS["codec"]), self.codec_combo); self.left_form_layout.addRow(self.create_help_label("해상도", HELP_TEXTS["resolution"]), self.resolution_combo); self.left_form_layout.addRow(self.create_help_label("GPU 프리셋", HELP_TEXTS["preset"]), self.preset_option_combo); self.left_form_layout.addRow(self.create_help_label("압축 방식", HELP_TEXTS["rate_control"]), self.rate_control_combo); self.left_form_layout.addRow(self.create_help_label("품질/비트레이트", HELP_TEXTS["quality_value"]), self.quality_edit)
        self.right_form_layout = QFormLayout(); self.right_form_layout.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapAllRows)
        self.right_form_layout.addRow(self.create_help_label("파일 접미사", HELP_TEXTS["suffix"]), self.suffix_edit); self.right_form_layout.addRow(self.create_help_label("오디오 옵션", HELP_TEXTS["audio_option"]), self.audio_option_combo); self.right_form_layout.addRow(self.create_help_label("출력 포맷", HELP_TEXTS["output_format"]), self.output_format_combo); self.right_form_layout.addRow(self.create_help_label("동시 작업 수", HELP_TEXTS["parallel_jobs"]), self.parallel_jobs_spinbox)
        main_settings_layout.addLayout(self.left_form_layout); main_settings_layout.addLayout(self.right_form_layout); main_layout.addWidget(settings_group)
        progress_group = QGroupBox("진행 상황"); progress_layout = QVBoxLayout(progress_group); progress_layout.addWidget(QLabel("전체 진행률 (완료된 파일 수)")); progress_layout.addWidget(self.overall_progress_bar); progress_layout.addWidget(self.log_edit); main_layout.addWidget(progress_group)
        button_layout = QHBoxLayout(); button_layout.addWidget(self.start_button); button_layout.addWidget(self.stop_button); main_layout.addLayout(button_layout)
    ...
    def stop_conversion(self):
        logging.info("사용자가 변환 중지를 요청했습니다.")
        ...
        self.set_controls_enabled(True); self.stop_button.setEnabled(False)
        self.statusBar().showMessage("작업이 중단되었습니다.", 5000); self.overall_progress_bar.setFormat("중단됨")
        self.append_log("모든 변환 작업이 중단되었습니다.")
        
    # 이하 다른 메소드들은 이전 최종 안정화 버전과 동일합니다.
    def apply_preset(self, preset_name):
        if preset_name == "사용자 정의": return
        ...
        try:
            ...
            self.update_suffix(settings["codec"])
        finally:
    ...
    def on_setting_changed(self, *args):
        ...
        for name, preset_values in PRESETS.items():
            ...
            if is_match:
                self.preset_combo.blockSignals(True); self.preset_combo.setCurrentText(name); self.preset_combo.blockSignals(False)
                match_found = True; break
        if not match_found and self.preset_combo.currentText() != "사용자 정의":
            self.preset_combo.blockSignals(True)
            if self.preset_combo.findText("사용자 정의") == -1: self.preset_combo.insertItem(0, "사용자 정의")
            self.preset_combo.setCurrentText("사용자 정의"); self.preset_combo.blockSignals(False)
    def load_app_settings(self):
        ...
        if preset_name != "사용자 정의" and preset_name in PRESETS: self.preset_combo.setCurrentText(preset_name)
        else:
            if self.preset_combo.findText("사용자 정의") == -1: self.preset_combo.insertItem(0, "사용자 정의")
...
PRESETS = {
    ...
    "Apple 기기 호환 (1080p)": {"codec": "H.265 (HEVC) - GPU", "resolution": "1080p", "preset_option": "p5", "rate_control": "CQP", "quality_value": "24", "audio_option": "원본 유지 (Passthrough)", "output_format": "mp4", "parallel_jobs": 2},
