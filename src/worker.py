...
class FileConversionTask(QRunnable):
    def __init__(self, input_file, settings, signals):
        super().__init__(); self.input_file = Path(input_file); self.settings = settings; self.signals = signals; self.process = None
    def run(self):
        try:
            ...
            ffmpeg_cmd = self.build_ffmpeg_command(ffmpeg_path, self.input_file, output_file); logging.info(f"실행할 FFmpeg 명령어: {' '.join(ffmpeg_cmd)}")
            startupinfo = None
            if os.name == 'nt': startupinfo = subprocess.STARTUPINFO(); startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
            ...
            self.process.wait()
            if self.process.returncode == 0: self.signals.log.emit(f"완료: '{output_file.name}' 변환 성공."); self.signals.finished.emit(self, True)
            elif self.process.returncode > 0: self.signals.log.emit(f"오류: '{self.input_file.name}' 변환 실패 (오류 코드: {self.process.returncode})."); self.signals.finished.emit(self, False)
            else: self.signals.finished.emit(self, False)
    ...
    def build_ffmpeg_command(self, ffmpeg_path, input_file, output_file):
        ...
        if s['output_format'] == 'mp4': ffmpeg_cmd.extend(['-movflags', '+faststart'])
